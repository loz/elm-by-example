...
import List exposing ((::), all, filter, head, isEmpty, length, map, tail)
  ...
  NotFinishedGame player moves -> 
    let fields = [
      ...
      {col=3, row=2}
        ...
        newField' = case newField of
          Nothing -> {row=0, col=0} -- head now return a Maybe
          Just x -> x
...
undoMoves state =
  case state of
    ...
    NotFinishedGame player (_::(_::t)) ->
      NotFinishedGame player t
    FinishedGame _ _ -> state
