...
import Graphics.Element exposing (Element, container, down, flow, layers, middle, right, spacer, leftAligned)
...
drawMoves state =
  let moveSign player = group (case player of
              X -> let xline = filled black (rect 5 64)
                   in [
                     rotate (degrees 45) xline,
...
view state =
  flow down [
    ...
    container 300 60 middle <| leftAligned <| Text.fromString(stateDescription state),
